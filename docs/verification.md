...
本记录对应 2026-07-27 的工作流重构，覆盖总控与角色 Skills、YAML/JSON Schema 契约、Python 编译/调度工具、项目文档初始化、Skill 安装器、Unity Workflow Toolkit 源码与最小测试宿主。
...
结果：`163 passed`。覆盖契约校验、确定性 Job 编译、DAG、跨进程锁、状态机、CLI、初始化器、安装器、Windows 实机捕获脚本、深度质量门求值、不可变资源登记合并、端到端视觉门禁和 Toolkit 静态结构。
...
结果：`6 passed`。已确认模块/测试文件齐备、asmdef 可解析且名称唯一、UnityHost 固定 unity-mcp v10.1.0、四个自定义工具名称准确、C# 文件均小于 1000 行且类型/方法具备 XML 摘要，并覆盖新增 Toolkit 安全约束的静态结构。
