...
- 场景性能符合预算。
- 缺失引用检查通过。
- Editor 固定机位截图和审查报告齐全。
- 用户确认 Unity 内最终效果。

冻结后输出场景交付 Manifest、批准的效果图、Unity Editor 截图、视觉差异报告、测试报告和研发期静态预算报告。G2 `PASS` 后的 G3 才输出实机截图和目标设备性能报告。
...
- 游戏可读性。
- UI 层级、文字和安全区。
- 研发阶段检查效果图与 Editor 实现截图差异；G3 检查效果图与实机截图差异。
- 用户最终确认。
...
每个视觉任务保留生成 Prompt、参考图角色、模型路径、候选版本、审查意见、批准记录、文件哈希、Unity Importer 配置和 Editor 截图；实机截图仅由 G2 `PASS` 后的 G3 设备验证任务保留。
