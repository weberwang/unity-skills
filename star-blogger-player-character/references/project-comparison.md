...
本规范把人物合同与当前 Star Blogger Unity 项目的差异收敛为可复查的机器记录。不得用“看起来差不多”、旧 PASS 摘要、文件数量或单张缩略图代替当前项目基线。
...
- `character-contract.md` 中的权威文件、17 项类别/标签/文件名和稳定运行时路径。
...
4. 使用 `templates/project-comparison.yaml` 生成 `CHARACTER_PROJECT_COMPARISON` 记录，并用 `schemas/project-comparison.schema.json` 校验。记录必须绑定来源修订、项目状态版本、全部权威输入、17 项实际映射、四个 Unity 资产和项目对照检查。
...
9. 基线有任一 `MISSING` 或 `UNREADABLE` 时把汇总标记为 `BLOCKED`；存在可审计差异时标记 `DIFFERENCES_FOUND`；全部一致时才标记 `MATCHED`。基线只说明当前状态，不批准生产候选。
...
输出一份符合 `schemas/project-comparison.schema.json` 的 YAML，建议路径为 `Artifacts/Visual/P4/g1-v0.6/p3-002/v5/comparisons/p3-002-project-baseline-<revision>.yaml`。所有路径使用项目相对正斜杠格式；每个文件绑定真实 SHA-256，Unity 资源同时绑定 `.meta` 与 32 位小写 GUID。
...
- 项目根、运行时文件、`.meta`、GUID 或 Unity 引用缺失/不可读：保持 `BLOCKED`，先恢复项目事实，不生成候选。
