...
4. Visual Bible 候选按多级漏斗审核：F0 校验契约、证据和硬约束，F1 主责筛选至多三个方向，F2 由视觉一致性、游戏可实现性、UX/可读性三个不同审查者收敛到唯一方向；主代理只合并为无冲突、可执行且标明优先级的修改清单。
5. F2 收敛后，先用 `$unity-game-grilling` 对唯一 Visual Bible 候选逐项确认方向取舍并生成绑定记录，再由 F3 请求用户确认全局视觉。只有 F0-F3、拷问记录与用户批准均有效，才可把状态设为 `APPROVED`；后续风格变更必须创建新版本并使受影响场景回到待复核。
6. 每个场景按 P0-P5 严格串行推进，任何阶段不得越级，也不得以口头确认替代绑定版本与 SHA-256 的状态证据：
   ...
   - **P1 高保真候选生成**：游戏内画面与 UI 可分别出图，但必须共享同一场景结构版本和 Visual Bible；状态只允许 `HIGH_FIDELITY_DRAFT -> HIGH_FIDELITY_GENERATING -> HIGH_FIDELITY_GENERATED`。P1 记录候选全集，不设置用户送审确认门。
   - **P2 多级漏斗审核**：F0 自动预检全部 P1 候选，F1 主责筛选至多三个，F2 由视觉一致性、Unity 可实现性、UX/可读性三个不同审查者收敛到唯一候选；用 `$unity-game-grilling` 冻结 F0-F2 后，F3 只向用户提交唯一推荐、合并结论和未消除风险。状态为 `SCREENING -> SPECIALIST_REVIEWING -> AWAITING_USER -> APPROVED | CHANGES_REQUIRED`。候选内容变化必须生成新的 P1 修订并从 F0 重跑；不得在 F3 替换或扩增候选。
...
11. P5 的清理验证 `PASS` 并进入 `ASSEMBLY_VERIFIED` 后，读取 `mcpforunity://scene/cameras` 核对固定机位，只在 Unity Editor/Game View 的批准参考分辨率与状态下生成 Editor 视觉证据；Editor 候选同样按 F0-F3 漏斗收敛，F2 由三个独立审查者完成，唯一候选经 `$unity-game-grilling` 后才在 F3 请求用户批准。研发阶段禁止安装或启动 Standalone/Player、真机或等价设备；G2 `PASS` 后才由 G3 生成候选包、捕获 `runtime-visual-evidence` 并执行设备视觉审查。
...
- F0 可批量运行；F1 最多保留三个候选。三个 F2 审查代理独立输出，不能互相覆盖结论；主代理只合并修改清单，开发代理不得批准自己的候选。F3 只接收 F2 的唯一输出。
...
研发阶段输出 `visual-bible.yaml`、P0 `prefab-structure.yaml`、P1 `image-generation.yaml` 2.0 候选集、P2 `visual-review.yaml` 2.0 漏斗记录、P3 `split-plan.yaml`、P4 聚合验证、P5 `prefab-assembly.yaml`、清理验证与 Editor 固定机位 `visual-capture.yaml`。带平台身份的 `runtime-visual-evidence.yaml` 只能在 G2 `PASS` 后由 G3 输出，并必须引用当前 `g2.development-complete`。

## 通过条件

- Visual Bible 与场景视觉均完成 F0-F3，F2 三类独立审查收敛到唯一候选且 F3 用户批准有效；当前场景严格完成 P0-P5，清理验证为 `PASS`，且每道门禁引用同一条未失效版本链。
...
- 用户在 P0、P2/F3、P3 或最终实机门禁拒绝时保留版本与原因，回到最早受影响阶段产生新修订，并按失效规则重跑全部下游阶段。
