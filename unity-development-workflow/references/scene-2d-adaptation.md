...
目标场景使用正交摄像机、2D 世界和 UI Toolkit，且需要同时覆盖多个桌面窗口宽高比时读取。该流程是每个 2D 场景小循环的必做项，不替代全局视觉、玩法或 UI 审查。
...
4. 对每个目标分辨率验证：玩法摄像机尺寸、UI match、边带方向、无黑边、边带无交互、交互元素未越出参考安全区。
5. 捕获游戏、UI 和 Windows 实机证据。只有所有分辨率均为 PASS，且 Unity 配置、EditMode 与逐分辨率运行截图三类结构化证据全部通过时，契约状态才可标记为 `VERIFIED`。
...
- `runtime-2d-adaptation-screenshot`：每个声明分辨率恰好一份，记录构建版本、截图 SHA-256、无黑边、边带可见与交互安全检查结果。

`scene-manifest` 的 `dimension: 2D` 必须引用上述契约；`dimension: 3D` 不得伪造该引用。G2 的 `scenes.2d-adaptation-verified` 必须提交当前拆分中全部已批准场景的 manifest，逐项深验其中的 2D 适配；没有 2D 场景时仍核对场景全集，但不要求 3D 场景伪造适配证据。
...
- EditMode、目标分辨率截图和 Windows 实机证据均引用当前场景版本；未运行项没有被推断为通过。
