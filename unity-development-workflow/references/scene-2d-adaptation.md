...
4. 在场景小循环中对每个声明分辨率验证玩法摄像机尺寸、UI match、边带方向、无黑边、边带无交互和交互安全区；Unity 配置、数学、EditMode 与参考环境运行结果全部通过后，才可把适配契约标记为 `VERIFIED`，供 G2 模块与场景验收消费。这里的 `VERIFIED` 只证明适配实现有效，不代表最终设备验收。
