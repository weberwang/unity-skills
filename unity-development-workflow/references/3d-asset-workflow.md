...
2. **P1/P2 确认目标外观**：基于 P0 生成高保真游戏/UI 候选集；F0 自动预检，F1 主责筛选至多三个，F2 由视觉一致性、Unity 可实现性、玩法/可读性代理独立审查并收敛唯一候选，F3 再请求用户批准。候选内容变化时必须回到新 P1 修订并从 F0 重跑，不得直接沿用旧批准。
...
- Prefab 在目标场景、主开发平台构建和各批准平台适配验证中可复现，Console、功能、物理、视觉与性能证据为当前版本。
