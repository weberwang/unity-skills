...
7. 审查代理无权代替用户门禁。用户批准必须绑定当前对象、版本和哈希；无当前证据、状态未知或任一前置失败时，所有代理均必须停止推进。
8. 同一时间只允许一个主场景进入正式验收；当前场景小循环未冻结前，其他场景只能只读准备，不得写入 Unity、共享状态或正式资源。
9. 默认使用当前工作区和严格路径所有权；只有用户明确要求时才允许用 Worktree 隔离多个并发写入者。Git/Worktree 不参与质量门或状态机。
...
- L2 严格执行严格执行策略中的 Unity MCP 写入前后校验：写前核验实例、Editor、门禁、版本批准、基线和锁；写后等待稳定并复读对象、场景、Console 与登记，运行最小验证并保存证据。
- 未获得锁、实例不匹配、场景未保存或任一写前/写后校验失败时不得启动或推进写任务。
