...
只分析 F4 已确认的高保真原图。对全部可见元素完整编号，并分类为：独立位图、批准复用、文本、Unity 图元/程序绘制、UXML/USS/矢量、材质/VFX、3D。每项记录结构节点、状态变体、尺寸、透明度、导入和验证规格；漏标、未分类或无节点映射不得送审。F4 确认资产地图后，才逐项独立生成或重绘、导入并验证单图。
